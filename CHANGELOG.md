...
**e2e** — scenario 43 covers hot-reload (`[backends.*]` add/edit/delete take effect on the next request without a restart) with 7 assertions including a delete-then-fall-back-to-default and audit-log spot-checks. Scenario 44 covers the 5xx fallback contract with 5 assertions: a `503` on primary triggers a fallback walk to the backup (returns 200), a rule without fallback surfaces the primary's `503` unchanged, `/v1/messages` walks the same chain (Anthropic envelope intact), and the Console PUT rejects an unknown fallback or a self-fallback (400). Total `tools/e2e.sh` assertions: 231 (was 219 after PR #48).
