...
| `src/bin/nanoguard-admin.rs` | Offline user-management CLI for the Console DB (e.g. `set-password`, `list-users`); shares `[budget].db_path` and `console::auth` with the in-process console listener. |
...
Allowed: axum, tower-http, tokio, reqwest, rusqlite, rustls, serde, serde_json, serde_yaml, toml, toml_edit, aho-corasick, regex, jsonschema, iword-rs (legacy), unicode-normalization, sha2, chrono, async-trait, async-stream, futures-util, bytes, once_cell, anyhow, thiserror, tracing, tracing-subscriber, arc-swap, subtle, rand
