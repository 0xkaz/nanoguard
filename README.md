...
The Console **Backends** tab (admin only) shows the current pool, the routing table, and lets you add or remove entries from a UI. The same tab carries a **Routing** section that edits `[routing]` (default backend + first-match-wins rules) with ↑ / ↓ buttons to reorder rules — order is significant. The mutation rewrites `nanoguard.toml` on disk via a TOML round-trip (unrelated sections and comments preserved). Both `[backends.*]` and `[routing]` are **hot-reloadable**: saving from the Backends or Routing section fires a reload trigger and the next request sees the new pool / rules without a process restart. In-flight requests hold their own `Arc<AppState>` snapshot, so the prior pool's per-backend `reqwest::Client` stays alive until those requests finish — no orphaning. Per-rule failover is opt-in: a routing rule may carry a `fallback = ["backup-1", ...]` list, and on a network error or upstream status >= 500 the proxy tries each fallback once, in order, before surfacing the failure. There is **no** automatic / global failover — only the labels explicitly listed on the matched rule are tried. See `docs/design/multi-backend-routing.md > State management` for the full reload contract.
...
- Automatic / health-checked / cost-aware failover. The shipped `fallback = [...]` is the bare minimum (try-once on 5xx or network error, in the order the operator declared). Smart routing — health probes, circuit breakers, per-backend retry budgets — stays LiteLLM's territory.
